...
const severityStyles: Record<GapSeverity, { badge: string; dot: string }> = {
  low: { badge: "bg-warning/10 text-warning border-warning/25", dot: "bg-warning" },
  medium: { badge: "bg-warning/20 text-warning border-warning/40", dot: "bg-warning" },
  high: { badge: "bg-destructive/10 text-destructive border-destructive/25", dot: "bg-destructive" },
};
      ...
      {status === "success" && result ? (
        <div className="mt-6 grid gap-6">
          ...
          <Card className="rounded-2xl p-5 sm:rounded-3xl sm:p-8">
            ...
            {/* Summary */}
            <p className="mt-6 rounded-2xl bg-muted p-4 text-sm leading-7">
              {result.summary}
            </p>
          </Card>
            ...
            <Card className="rounded-2xl p-5 sm:rounded-3xl sm:p-6">
              ...
              <ul className="mt-5 grid gap-6">
                {result.recommended_updates.map((u) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="min-w-0 font-semibold">{u.section}</p>
                      <Badge
                    ...
                    {u.example_rewrite ? (
                      <p className="mt-3 rounded-xl bg-background p-3 text-xs leading-5 text-muted-foreground ring-1 ring-border">
                        {u.example_rewrite}
                      </p>
                    ) : null}
                  </li>
